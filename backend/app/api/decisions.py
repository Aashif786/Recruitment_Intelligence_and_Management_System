...
def make_hiring_decision(
    ...
    # Update interview status sync
    previous_interview_status = "completed"
    if application.interview:
        previous_interview_status = application.interview.status
        if decision_data.decision == "hired":
            application.interview.status = "approved"
        else:
    ...
    if decision_data.decision == "hired":
        application.status = "hired" # Maps to Face-to-Face in frontend
    else:
        application.status = "rejected_post_interview"
    db.commit()
    
    # Send email notification to candidate IDEMPOTENTLY
    candidate_email = application.candidate_email
    job = application.job
    
    # Only fire email if interview status actually transitioned to approved now
    if decision_data.decision == "hired" and previous_interview_status != "approved":
        background_tasks.add_task(send_hired_email, candidate_email, job.title, application.interview)
    elif decision_data.decision == "rejected" and previous_interview_status != "rejected":
        # Pass False = rejected post interview
