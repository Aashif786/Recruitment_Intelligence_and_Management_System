...
export default function ReportsPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'aptitude_round':
      case 'ai_interview':
      case 'ai_interview_completed': return 'bg-accent/10 text-accent border-accent/20'
      case 'physical_interview': return 'bg-secondary/10 text-secondary border-secondary/20'
      default: return 'bg-muted text-muted-foreground border-border'
    }
  }
  ...
  const filteredReports = useMemo(() => {
    return reports.filter(report => {
      ...
      // Basic search — match filename, primary_skill, candidate_name, and candidate_email
      const matchesSearch = searchQuery === '' ||
        report.filename.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (report.candidate_profile.primary_skill || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (report.candidate_profile.candidate_name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (report.candidate_profile.candidate_email || '').toLowerCase().includes(searchQuery.toLowerCase())
